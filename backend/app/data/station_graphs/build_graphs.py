...
def build_station_graph_new_spec(station_code, station_info):
    ...
    # ----------------------------------------------------------
            ...
            exit_node_id = (
                f"{station_code}_1F_EXIT_{exit_no_clean}"
            )
            ...
            if exit_node_id not in nodes_dict:
                nodes_dict[exit_node_id] = {
                    ...
                    "description": (
                        f"{station_name} "
                        f"{exit_no_clean}번 출구"
                    ),
                }

    # ----------------------------------------------------------
    # 실제 shuttleSection 방향을 그대로 반영
    #
    # B2 -> 1F
    # B2 CONCOURSE -> EXIT
    #
    ...
    # ----------------------------------------------------------
            ...
            plat_node_id = (
                f"{station_code}_LINE_01_"
                f"{from_floor}_"
                f"{type_code}_PLATFORM_"
                f"{idx + 1:03d}"
            )
